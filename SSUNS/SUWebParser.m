...
+(void)loadCommitteesListWithResponse:(SUBlockResponse)suresponse andError:(SUBlockError)error{
    ...
    NSURL *commURL = [NSURL URLWithString:[ssunsPre stringByAppendingString:@"/committees-list"]];
    
    NSOperationQueue *queue = [[NSOperationQueue alloc] init];
    NSURLRequest* thisRequest = [NSURLRequest requestWithURL:commURL cachePolicy:NSURLRequestReturnCacheDataElseLoad timeoutInterval:10];
    [NSURLConnection sendAsynchronousRequest:thisRequest queue:queue completionHandler:^(NSURLResponse *response, NSData *data, NSError *error) {
...
+(void)loadCommittee:(NSString*)committeeString withResponse:(SUBlockResponse)suresponse andError:(SUBlockError)error{
    ...
    NSURL *commURL = [NSURL URLWithString:committeeString];
...
+(void)loadItinerarywithResponse:(SUBlockResponse)suresponse andError:(SUBlockError)error{
    ...
    NSURL *itinUrl = [NSURL URLWithString:[ssunsPre stringByAppendingString:@"/itinerary"]];
    
    NSOperationQueue *queue = [[NSOperationQueue alloc] init];
    NSURLRequest* thisRequest = [NSURLRequest requestWithURL:itinUrl cachePolicy:NSURLRequestReturnCacheDataElseLoad timeoutInterval:10];
    [NSURLConnection sendAsynchronousRequest:thisRequest queue:queue completionHandler:^(NSURLResponse *response, NSData *data, NSError *error) {
...
+(void)loadMapWithResponse:(SUBlockResponse)suresponse andError:(SUBlockError)error{
    ...
    NSURL *commURL = [NSURL URLWithString:[ssunsPre stringByAppendingString:@"/hotel-dir"]];
